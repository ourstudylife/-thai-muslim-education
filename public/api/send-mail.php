...
$lineId = filter_var($data['lineId'] ?? '', FILTER_SANITIZE_STRING);
$preferredMethod = filter_var($data['preferredMethod'] ?? '', FILTER_SANITIZE_STRING);
...
// Validate 'From' header based on server config? 
// Hostinger usually requires the 'From' address to be from the same domain.
// noreply@thaimuslimeducation.com is likely safe if the domain matches.
...
if (mail($to, $subject, $emailBody, $headers)) {
    ...
    echo json_encode(['success' => false, 'error' => 'Failed to send email via server mail().']);
}
?>